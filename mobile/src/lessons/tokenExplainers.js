...
// Rate keys that map this token to a field in /api/rates response
const RATE_KEY = {
  [MSOL_MINT]:    "marinade_apy",
  [JITOSOL_MINT]: "jitosol_apy",
  [INF_MINT]:     "sanctum_inf_apy",
  [BSOL_MINT]:    null,
};

export const TOKEN_EXPLAINERS = {
  SOL: {
    emoji: "◎",
    name: "Solana",
    ...
    action: "Your SOL can earn ~7–8% APY passively by staking it. You don't have to sell it — just put it to work.",
    color: "#9945FF",
    isStaking: false,
  ...
  [MSOL_MINT]: {
    ...
    how: "The mSOL exchange rate rises over time because staking rewards are baked into it. Holding mSOL is the same as holding SOL + earning ~7% APY.",
    action: "You can trade mSOL on any DEX, use it as collateral on Kamino, or unstake it back to SOL anytime — no lockup.",
    color: "#60A5FA",
    isStaking: true,
  ...
  [JITOSOL_MINT]: {
    ...
    tagline: "SOL staking + MEV rewards",
    what: "jitoSOL is Jito's liquid staking token. Like mSOL, you stake SOL and get a receipt — but Jito adds MEV (maximal extractable value) rewards on top.",
    ...
    action: "jitoSOL is widely accepted as collateral on Kamino and other lending protocols. You can use it the same way as mSOL.",
    color: "#A78BFA",
    isStaking: true,
  ...
  [INF_MINT]: {
    ...
    action: "INF can be used as collateral and traded freely. Sanctum allows near-instant liquidity between any LST — no waiting for unstaking.",
    color: "#4ADE80",
    isStaking: true,
    relatedLessonId: "lst-explained",
    relatedLessonLabel: "Learn: mSOL vs jitoSOL vs INF",
  ...
  [BSOL_MINT]: {
    ...
    action: "bSOL can be swapped freely on Jupiter and used in select DeFi protocols on Solana.",
    color: "#FB923C",
    isStaking: true,
    relatedLessonId: "liquid-staking",
    relatedLessonLabel: "Learn: Liquid staking explained",
  ...
  [USDC_MINT]: {
    ...
    action: "You can lend USDC on Kamino to earn ~5–8% APY, or use it to DCA into SOL automatically with Jupiter.",
    color: "#2775CA",
    isStaking: false,
  ...
  [USDT_MINT]: {
    ...
    what: "USDT (Tether) is the world's largest stablecoin by volume. Like USDC, 1 USDT = ~$1. It's commonly used for trading and transfers.",
    ...
    action: "You can swap USDT to USDC for more transparency, or lend it on Kamino to earn yield.",
    color: "#26A17B",
    isStaking: false,
...
export const UNKNOWN_EXPLAINER = {
  emoji: "🪙",
  name: null,
  ...
  action: 'Tap "Ask Homie" to get a plain-English explanation of exactly this token.',
...
export function getExplainer(mint) {
